...
export BITCOINLITECLI=${BUILDDIR}/qa/pull-tester/run-bitcoinlite-cli
export BITCOINLITED=${REAL_BITCOINLITED}
...
if [ "x${ENABLE_BITCOINLITED}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and bitcoinlited must all be enabled"
fi

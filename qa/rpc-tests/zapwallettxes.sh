...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars BITCOINLITED and BITCOINLITECLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

BITCOINLITED=${BITCOINLITED:-${1}/bitcoinlited}
CLI=${BITCOINLITECLI:-${1}/bitcoinlite-cli}
...
$BITCOINLITED -zapwallettxes=1 $B1ARGS &
B1PID=$!
$BITCOINLITED -zapwallettxes=2 $B2ARGS &
B2PID=$!

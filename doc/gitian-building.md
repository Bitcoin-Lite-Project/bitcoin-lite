...
*Setup instructions for a gitian build of BitcoinLite using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BitcoinLite
...
- [Building BitcoinLite](#building-bitcoinlite)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-bitcoinlite-host/README.md).
...
Clone the git repositories for bitcoinlite and gitian and then checkout the bitcoinlite version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/bitcoinlite/bitcoinlite
cd bitcoinlite
...
Follow the instructions in [doc/release-process.md](release-process.md) in the bitcoinlite repository
under 'Fetch and build inputs' to install sources which require manual intervention. Also follow
the next step: 'Seed the Gitian sources cache', which will fetch all necessary source files allowing
for gitian to work offline.

Building BitcoinLite
----------------

To build BitcoinLite (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the bitcoinlite repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitcoinlite/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/bitcoinlite/bitcoinlite
...
For example:
```bash
URL=https://github.com/laanwj/bitcoinlite.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit bitcoinlite=${COMMIT} --url bitcoinlite=${URL} ../bitcoinlite/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitcoinlite=${COMMIT} --url bitcoinlite=${URL} ../bitcoinlite/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitcoinlite=${COMMIT} --url bitcoinlite=${URL} ../bitcoinlite/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitcoinlite-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitcoinlite-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/bitcoinlite-build.assert
```
...
[bitcoinlite/gitian.sigs](https://github.com/bitcoinlite/gitian.sigs/) repository, or if that's not possible create a pull
